...
class MainTest {
    ...
    fun `find horizontal movers`() {
        ...
        assertEquals(setOf(Position(5,0),Position(4,1)),  map.horizontalMovers(map.maxX))
    }
    @Test
    fun `find vertical movers`() {
        val map = sampleData.subList(0,2).parse()
        assertEquals(setOf(Position(0,0),Position(8,0),Position(1,1),Position(2,1),Position(6,1)),  map.verticalMovers(map.maxY))
    }
    @Test
    fun `update map with horizontal movers`() {
        ...
        map.updateWithHorizontalMovers(movers, map.maxX)
    ...
    fun `update map with vertical movers`() {
        ...
        map.updateWithVerticalMovers(movers, map.maxY)
