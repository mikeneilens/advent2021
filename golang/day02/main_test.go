...
func Test_createListOfCommands_forForward(t *testing.T) {
	var data = `forward 5`
	commands := createListOfCommands(data, part1FunctionCreator)
	sub := submarine{1, 2, 3}
	commands[0](&sub)
...
down 8
forward 2`
	result := day02(sampleData, part1FunctionCreator)
	if result != 150 {
...
func Test_part01_using_PuzzleInput(t *testing.T) {
	result := day02(PuzzleInput, part1FunctionCreator)
	if result != 1383564 {
...
func Test_part02_using_PuzzleInput(t *testing.T) {
	result := day02(PuzzleInput, part2FunctionCreator)
