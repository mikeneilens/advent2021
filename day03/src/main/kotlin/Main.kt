fun mostCommonBitInColumn(list: List<String>, column: Int) =
    if (list.map{it[column]}.count{it=='0'} > list.size /2) '0' else '1'

fun gamma(list:List<String>):String =
    (0 until list[0].length).map { ndx -> mostCommonBitInColumn(list, ndx) }.joinToString("")
...
fun calcRating(list:List<String>, rating:Rating, ndx:Int = 0):String =
    if (list.size == 1) list.first()
        ...
        val filtered = if (mostCommonBitInColumn(list, ndx) == rating.bit) list.filter { it[ndx] == '1' } else list.filter { it[ndx] == '0' }
        calcRating(filtered, rating, ndx + 1)
    }
