...
class MainTest {
    @Test
    fun `parse sampledata1 input into map`() {
        val map = sampleData1.partOne()
        assertEquals(39, map.values.count { it })
    }
    @Test
    fun `parse sampledata2 input into map`() {
        val map = sampleData2.partOne()
        assertEquals(590784, map.values.count { it })
    }

    @Test
    fun `parse puzzleInput input into map`() {
        val map = puzzleInput.partOne()
        assertEquals(556501, map.values.count { it })
    }
    ...
    fun `part two with sample data`() {
        val result = sampleData3.map(String::toCuboid).partTwo()
        val volumes:Long = result.sumOf { it.volume }

        assertEquals(2758514936282235, volumes)
    }
    @Test
    fun `part two`() {
        val result = puzzleInput.map(String::toCuboid).partTwo()

        val volumes:Long = result.sumOf { it.volume }

        assertEquals(1217140271559773, volumes)
    }
    @Test
